...
main (void)
{
    DDRC = 0x0f; // C0-C3 output for LEDs
 	PORTC =~0; // All LED off (com anodes)
    ...
    for (;;)
    {
        if (irmp_get_data (&irmp_data))
        {
			...
			// New for switching an RGB LED, 4 colors and off...
			switch (irmp_data.address){
				case 0xFF00: 
					switch (irmp_data.command){
						case 0x0040: PORTC = OFF;
						break;
						case 0x0043: PORTC = ~RT;
						break;
						case 0x0044: PORTC = ~GN;
						break;
						case 0x0046: PORTC = ~BL;
						break;
						case 0x0015: PORTC = ~WS;
						break;
						}
